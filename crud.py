...
def Create():
    if  FIRSTNAME.get() == "" or LASTNAME.get() == "" or GENDER.get() == "" or ADDRESS.get() == "" or USERNAME.get() == "" or PASSWORD.get() == "":
        ...
        cursor.execute("INSERT INTO `members` (firstname, lastname, gender, address, user, password) VALUES(%s, %s, %s, %s, %s, %s)",
                       (str(FIRSTNAME.get()), str(LASTNAME.get()), str(GENDER.get()), str(ADDRESS.get()), str(USERNAME.get()), str(PASSWORD.get())))
        tree.delete(*tree.get_children())
        cursor.execute("SELECT * FROM `members` ORDER BY `lastname` ASC")
...
def Update():
    ...
    if GENDER.get() == "":
        ...
        tree.delete(*tree.get_children())
        cursor.execute("UPDATE `members` SET `firstname` = %s, `lastname` = %s, `gender` = %s,  `address` = %s,  `user` = %s, `password` = %s WHERE `mem_id` = %s",
    ...
    if not tree.selection():
       txt_result.config(text="Please select an item first", fg="red")
        ...
        if result == 'yes':
            ...
            tree.delete(curItem)
            Database()
            cursor.execute("DELETE FROM `members` WHERE `id` = %d" % selecteditem[0])
            conn.commit()
            cursor.close()
            conn.close()
...
tree = ttk.Treeview(Right, columns=("ID", "Firstname", "Lastname", "Gender", "Address", "User", "Password"), selectmode="extended", height=500, yscrollcommand=scrollbary.set, xscrollcommand=scrollbarx.set)
scrollbary.config(command=tree.yview)
scrollbary.pack(side=RIGHT, fill=Y)
scrollbarx.config(command=tree.xview)
scrollbarx.pack(side=BOTTOM, fill=X)
tree.heading('ID', text="MemberID", anchor=W)
